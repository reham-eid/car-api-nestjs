...
describe('AuthService-Test', () => {
  ...
  it('throws an error message if email of user already used while sign-up', async () => {
    //overrides the find method {OLD-WAY}
    await service.signUp('rere@rere.com', 'qwr') 
    await expect(service.signUp('rere@rere.com', 'qwr')).rejects.toThrow(
      BadRequestException,
    );
  });

  it('throws an error message if sign-in is called with email of user thats not found ', async () => {
  ...
  it('throws an error message if an invalid password is provided while sign-in', async (done) => {

    await service.signIn('rere@we.com', '1')
    try {
      await expect(service.signIn('rere@we.com', 'P2assword#123')).rejects.toThrow(
        UnauthorizedException,
      );
    } catch (error) {
      done()
  ...
  it('sign-in with hashed & correct password ', async () => {
    /** OLD WAY (more static) */
